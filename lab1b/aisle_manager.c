...
void set_spaces(unsigned long* aisle, int index, unsigned short new_spaces) {
  ...
  if(new_spaces >> NUM_SPACES == 0) {	  
    unsigned short section_w_new_spaces = get_section(aisle, index) ^ (get_spaces(aisle,index));;
    section_w_new_spaces = section_w_new_spaces | (new_spaces);
    set_section(aisle, index, section_w_new_spaces);
  }
}
...
void set_id(unsigned long* aisle, int index, unsigned short new_id) {
  // TODO: implement this method
  if(new_id >> ID_SIZE == 0) {
    unsigned short section_w_new_id = get_section(aisle,index) ^ (get_id(aisle,index) << NUM_SPACES);
    section_w_new_id = section_w_new_id | (new_id << NUM_SPACES);
    set_section(aisle, index, section_w_new_id);
  }
}
...
void toggle_space(unsigned long* aisle, int index, int space_index) {
  ...
  spaces = spaces ^ ((unsigned short)1 << space_index);
...
unsigned short num_items(unsigned long* aisle, int index) {
  ...
  for(int i = 0; i < NUM_SPACES; i++) {
    if((spaces & ((unsigned short)1 << i)) != 0) {
      count++;
    }
  }
  return count;
...
void add_items(unsigned long* aisle, int index, int n) {
  // TODO: implement this method
  if((NUM_SPACES-num_items(aisle, index)) <= n) {
...
void rotate_items_left(unsigned long* aisle, int index, int n) {
  ...
  n = n%NUM_SPACES;
  for(int i = 0; i < n; i++) {
    if((spaces & ((unsigned short)1 << (NUM_SPACES-1))) != 0) {
      spaces = spaces << 1;
      spaces = spaces | 1;
    } else {
      spaces = spaces << 1;
...
void rotate_items_right(unsigned long* aisle, int index, int n) {
  // TODO: implement this method
  rotate_items_left(aisle, index, NUM_SPACES-(n%NUM_SPACES));
}
