...
void refill_from_stockroom() {
  ...
  for(int aisle = 0; aisle < NUM_AISLES; aisle++) {
    for(int section = 0; section < SECTIONS_PER_AISLE; section++) {
      unsigned short curStock = num_items(aisles+aisle, section);
      unsigned short curId = get_id(aisles+aisle, section);
      add_items(aisles+aisle, section, *(stockroom+curId));
      *(stockroom+curId) = *(stockroom+curId) - (num_items(aisles+aisle, section)-curStock);
    }
  }
}
...
int fulfill_order(unsigned short id, int num) {
  ...
  int count = 0;
  for(int aisle = 0; aisle < NUM_AISLES; aisle++) {
    for(int section = 0; section < SECTIONS_PER_AISLE && num != count; section++) {
      unsigned short curId = get_id(aisles+aisle, section);
      if(curId == id) {
	unsigned short prevStock = num_items(aisles+aisle, section);
        remove_items(aisles+aisle, section, num-count);
	count+=(prevStock-num_items(aisles+aisle, section));
      }
    }
  }
  if(count < num) {
    if(*(stockroom+id) <= num-count) {
      count+=*(stockroom+id);
      *(stockroom+id) = 0;
    } else {
      *(stockroom+id)-=(num-count);
      count = num;
    }
  }
  return count;
  ...
  for(int aisle = 0; aisle < NUM_AISLES; aisle++) {
    for(int section = 0; section < SECTIONS_PER_AISLE; section++) {
      if(id == get_id(aisles+aisle, section) && num_items(aisles+aisle, section) == 0) {
        return (unsigned short*)(aisles+aisle) + section;
      }
    }
  }
  return NULL;
...
unsigned short* section_with_most_items() {
  // TODO: implement this function
  int maxItems = num_items(aisles, 0);
  int maxAisle = 0;
  int maxSection = 0;
  for(int aisle = 0; aisle < NUM_AISLES; aisle++) {
    for(int section = 0; section < SECTIONS_PER_AISLE; section++) {
      int curItems = num_items(aisles+aisle, section);
  ...
  return (unsigned short*)(aisles+maxAisle) + maxSection;
}
