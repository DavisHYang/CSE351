...
void* mm_malloc(size_t size) {
  ...
  // try to search for a free block in the list
  ptr_free_block = search_free_list(req_size);
  if(ptr_free_block == NULL) {
	  // get more heap space if needed
	  request_more_space(req_size);
	  ptr_free_block = search_free_list(req_size);
  }
  // get size of block returned by search
  block_size = SIZE(ptr_free_block->size_and_tags);

  // set preceding_block_use_tag
  ...
  // Split the block if the block we found is too big
  if(block_size - req_size >= MIN_BLOCK_SIZE) {
	// Set the header tags of allocated block
	...
	// Update following block
	block_info* flw_blk = (block_info*)UNSCALED_POINTER_ADD(ptr_free_block, req_size); // retrieve following block
	flw_blk->size_and_tags = (block_size-req_size) | TAG_PRECEDING_USED; // header
	*((size_t*)UNSCALED_POINTER_ADD(ptr_free_block, block_size - WORD_SIZE)) = (block_size-req_size) | TAG_PRECEDING_USED; // footer

	// Insert excess back into list
	insert_free_block(flw_blk);

  } else { // No need to split
	// Set tags of allocated block
	ptr_free_block->size_and_tags = ptr_free_block->size_and_tags | TAG_USED; // header of allocated block
		
	// Update following block
	block_info* flw_blk = (block_info*)UNSCALED_POINTER_ADD(ptr_free_block, block_size); // retrieve following block
	flw_blk->size_and_tags = flw_blk->size_and_tags | TAG_PRECEDING_USED; // header
	size_t flw_blk_size = SIZE(flw_blk->size_and_tags); // size of flw_blk (to find footer location)
	*((size_t*)UNSCALED_POINTER_ADD(flw_blk, flw_blk_size-WORD_SIZE)) = flw_blk->size_and_tags; // footer
  }

  // return location of payload
...
void mm_free(void* ptr) {
  ...
  // retrieve head of block_info
  block_to_free = (block_info*)UNSCALED_POINTER_SUB(ptr, WORD_SIZE);

  // retrieve size of block from size_and_tags field
  payload_size = SIZE(block_to_free->size_and_tags);

  // retrieve next block
  following_block = (block_info*)UNSCALED_POINTER_ADD(ptr, payload_size);

  // update freed block tags (header + footer)
  block_to_free->size_and_tags = block_to_free->size_and_tags & ~TAG_USED; // header
  *((size_t*)UNSCALED_POINTER_ADD(block_to_free, payload_size-WORD_SIZE)) = block_to_free->size_and_tags; // footer

  // update following block tags
